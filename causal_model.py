...
class SCM(nn.Module):
    def __init__(self, d, A=None, scm_type='mlp'):
        super().__init__()
        self.d = d  # 把d赋值给self.d
        self.A_given = A  # 把A赋值给self.A_given
        self.A_fix_idx = A == 0  # 把A是否等于0的布尔值赋值给self.A_fix_idx
        self.A = nn.Parameter(torch.zeros(d, d))  # 创建一个d乘d的全零张量，并把它赋值给self.A

        # Elementwise nonlinear mappings
        if scm_type == 'linscm':  # 如果scm_type是'linscm'
            prior_net_model = lambda: InvertiblePriorLinear()  # 定义一个匿名函数，返回一个可逆的线性先验网络
            prior_net_enc_model = lambda x: InvertiblePriorInv(x)  # 定义一个匿名函数，返回一个可逆的先验网络的逆
        elif scm_type == 'nlrscm':  # 否则如果scm_type是'nlrscm'
            prior_net_model = lambda: InvertiblePWL()  # 定义一个匿名函数，返回一个可逆的分段线性网络
            prior_net_enc_model = lambda x: InvertiblePriorInv(x)  # 定义一个匿名函数，返回一个可逆的先验网络的逆
        else:  # 否则
            raise NotImplementedError("Not supported prior network.")  # 抛出一个未实现错误，提示不支持的先验网络

        for i in range(d):  # 对于0到d-1之间的每个整数i
            setattr(self, "prior_net%d" % i, prior_net_model())  # 用prior_net_model函数创建一个先验网络，并把它赋值给self.prior_neti属性
            setattr(self, "enc_net%d" % i, prior_net_enc_model(getattr(self,
                                                                       "prior_net%d" % i)))  # 用prior_net_enc_model函数和self.prior_neti属性创建一个编码网络，并把它赋值给self.enc_neti属性
    ...
    def forward(self, eps=None, z=None):
        if eps is not None and z is None:  # 如果eps不是None而z是None
            # (I-A.t)^{-1}*eps
            z = self.inv_cal(eps)  # 调用self.inv_cal函数，计算(I-A.t)^{-1}*eps，并赋值给z
            # nonlinear transform
            return self.prior_nlr(z)  # 返回调用self.prior_nlr后的结果
        else:  # 否则
            # f_2^{-1}(z)
            z = self.enc_nlr(z)  # 调用self.enc_nlr，计算f_2^{-1}(z)，并赋值给z
            # mask z
            z_new = self.mask(z)  # 调用self.mask，对z进行掩码操作，并赋值给z_new
            return z_new, z  # 返回z_new和z
